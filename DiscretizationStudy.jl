...
function DoOldDisc(B, U, m, A, initialTimeStep, X0::Zonotope{Float64,Vector{Float64},Matrix{Float64}})
    phiDict = Dict{Float64,  Matrix{Float64}}()
    discritezationDict = Dict{Float64, Zonotope{Float64,Vector{Float64},Matrix{Float64}}}()
    ...
    return (phiDict, discritezationDict, inputDiscritezationDict)
end
...
function cegarInputSystemNoOutputOldDisc(A, B, initialTimeStep, interval, X0::Zonotope{N,Vector{N},Matrix{N}}, U::Zonotope, constraint, Digits :: Integer, STRATEGY :: Integer) where {N}
    ...
    phiDict, discritezationDict, inputDiscritezationDict = DoOldDisc(B, U, m, A, initialTimeStep, X0)
