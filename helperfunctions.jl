...
function Φ₂(A, δ, invertible=false)
    let A = abs.(A)
        if invertible
            Aδ = A .* δ
            Φ = exp(Aδ) #fastExpm(Aδ; threshold=eps(Float64), nonzero_tol=eps(Float64))#; threshold=eps(Float64), nonzero_tol=eps(Float64))
            n = size(A, 1)
            N = eltype(A)
            ...
            C = _P_3n(A, δ, n)
            P = exp(C) #fastExpm(C; threshold=eps(Float64), nonzero_tol=eps(Float64))
            return _P₂_blk(P, n)
        end
    end
end
